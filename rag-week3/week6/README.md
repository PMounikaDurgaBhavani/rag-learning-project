...
- **Ticket replies.** `src/ticket_reply.py` drafts a reply using the same retrievers, distance gate, model, and decoding settings as `ask`. The policy the replies need (refund window, duplicate charges, Priority escalation) is a new article, `samples/article_08_refunds_escalation.md` (HC-008). It is imported into the corpus.
