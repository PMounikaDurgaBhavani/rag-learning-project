...
Nothing about a document's internal layout is assumed. Headings, tables,
frontmatter and section markers are all used when present and never
...
# Hand-written text formats, where a leading "---" block may be frontmatter.
# Uploads are never rewritten to add one - metadata lives in a sidecar.
...
# Frontmatter is only frontmatter when the file opens with a "---" fence,
# closes it on a line of its own, and the block in between parses to a
# mapping. Anything looser mistakes a horizontal rule, a row of dashes in
# an exported table, or a "---" separator inside pasted notes for metadata
# and silently swallows the first section of the document.
...
def split_frontmatter(text: str, allow: bool = True):
    """Return (metadata, remaining_text).

    ``allow`` is False for formats whose extracted text is generated by a
    parser (PDF, CSV, XLSX...): a table whose first cell is "---" must never
    be read as metadata.
    """
    if not allow:
        return {}, text
...
# How far into a file we look for a real heading before giving up.
TITLE_SCAN_LINES = 20


def derive_title(file_path: Path, content: str) -> str:
    """A short human title, without requiring any particular layout.

    A level-1 "# " heading is used when the document happens to have one;
    a "## " is a section inside a document, not its name.
    Otherwise the filename wins: the first line of a file is body text as
    often as it is a title, and promoting it produced titles that were a
    whole paragraph, a table header row, or a line of dashes — each of
    which then got prepended to every chunk of that document.
    """
    ...
    pretty = re.sub(r"[_\-]+", " ", file_path.stem).strip()
    pretty = re.sub(r"\s+", " ", pretty)
    return pretty or file_path.stem


def _default_metadata(file_path: Path, article_content: str) -> dict:
    return {
        "article_id": file_path.stem.upper().replace(" ", "-"),
        "title": derive_title(file_path, article_content),
...
def load_articles_from_files(directory=None):
    ...
    for file_path in sorted(directory.glob("*")):
        ...
        # Frontmatter is optional and only ever honoured on hand-written
        # text formats. Everything else keeps every byte of its content.
        metadata, article_content = split_frontmatter(
            raw_content,
            allow=file_path.suffix.lower() in TEXT_FRONTMATTER_EXTS,
        )
        article_content = article_content.strip()

        if not article_content:
            # The whole file was a frontmatter block; treat it as content
            # rather than dropping the document entirely.
            article_content = raw_content.strip()
            metadata = {}

        for key, value in _default_metadata(file_path, article_content).items():
